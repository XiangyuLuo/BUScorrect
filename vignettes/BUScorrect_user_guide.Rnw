...
\noindent To fit the BUS model, we first look at the data format. Two types of data format are allowed, the R list and the SummarizedExperiment object.\\ 
...
The sample sizes of each batch are 70, 80, and 70, respectively. Because it is a simulation data set, we know that all of the samples come from 3 subtypes. 
We can also prepare a SummarizedExperiment object named \Robject{BUSdata\_SumExp}.
